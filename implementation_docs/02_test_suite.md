...
61 test files run in parallel via ParallelTestRunner (`make test`, default 10 workers),
about 7100 passing assertions. The total is not exactly reproducible: 25 assertion
...
Observed 7103 and 7104 on one commit:
...
`test/extensive/` adds the two large solves in their own environment, outside `make test`
discovery: `make test-extensive`, ~4 minutes, see `02_status.md` for the measurements.

The certification subpackage adds 543 assertions (`make test-cert`):
...
Every v2 test file has a v3 counterpart, `test/extensive/` included.

`model_kit/symbolic_test.jl` is covered by `expression_test.jl` and `symbolic_utils_test.jl`,
except where v2's ModelKit carries machinery v3 does not have:

| v2 testset | why not ported |
|------------|----------------|
| SymEngine, Convert | `Expression` coefficients are `ComplexF64`; there is no `BigFloat`/`Rational`/`Int128` tower to round-trip |
| System (show) | `System` has no custom `show` |
