...
# Newton corrections shrink fast enough, mirroring the classic HC.jl `newton`.
...
Requires `m ≥ n` (square or overdetermined).
"""
struct NewtonCache
    x::FSVec{ComplexF64}
    Δx::FSVec{ComplexF64}
...
function NewtonCache(F::System)::NewtonCache
    m, n = size(F)
    m >= n || throw(
        ArgumentError("newton requires an m ≥ n system (got m = $m, n = $n)"),
    )
    return NewtonCache(
...
problem, so the method converges to a common zero when one exists. Computations
...
- `extended_precision::Bool = false`: evaluate `F(x)` in extended precision (matches v2's default).
